        ...
        for i in range(N):
            if t <= (2 / 3) * T:
                if np.random.rand() < 0.5:
                    X_new[i, :] = (
                        Best_P * (1 - t / T)
                        + (np.mean(X[i, :]) - Best_P) * np.random.rand()
                    )
...
def F_obj(x):
    ...
    t3 = 2 * (
        np.exp(-(x[0] ** 2) - (x[1] ** 2)) + np.exp(-(x[0] ** 2) - ((x[1] + 4) ** 2))
    )
    return t1 + t2 + t3
...
if __name__ == "__main__":
    ...
    Best_FF, Best_P, conv = AO(
        population_size, M_Iter, LB_fourpeak, UB_fourpeak, Dim, four_peak
    )

    print(
        "The best-obtained solution by AO is: [{:.8f}".format(Best_P[0]),
        "{:.8f}".format(Best_P[1]),
        "]",
    )
    print(
        "The best optimal values of the objective function found by AO is: {:.8f}".format(
            Best_FF
        )
    )
